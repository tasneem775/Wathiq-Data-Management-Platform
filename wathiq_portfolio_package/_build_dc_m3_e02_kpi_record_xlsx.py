# -*- coding: utf-8 -*-
"""Builds the supporting Excel calculation sheet for DC.M.3-E02
(DC-KPI-02 — نسبة تعيين ملاك البيانات).

All per-dataset rows (name, owner) are copied verbatim from the real evidence_repository
file DC.M.5.docx. No dataset name or owner name is invented. Saved alongside the
DC.M.3-E02 docx/pdf, same folder, same file name stem, .xlsx extension.
"""
from openpyxl import Workbook
from openpyxl.styles import Font, PatternFill, Alignment, Border, Side
from openpyxl.utils import get_column_letter

OUT_PATH = "06_DC_M3_KPI_Evidence/DC.M.3-E02_Data_Owner_Assignment_KPI_Calculation_Record.xlsx"
...
FORMULA_TEXT = "عدد المجموعات ذات المالك المحدد ÷ إجمالي المجموعات × 100"
FINAL_RESULT = f"{round(OWNED_DATASETS / TOTAL_DATASETS * 100)}%"  # "100%"

wb = Workbook()
ws = wb.active
ws.title = "احتساب DC-KPI-02"
...
# ============ Title banner ============
r = merged_text(r, "سجل احتساب مؤشر نسبة تعيين ملاك البيانات (DC-KPI-02)", 18, True, WHITE, fill=NAVY, height=32)
...
r = spacer(r, 6)
r = merged_text(r, "سجل احتساب مؤشر نسبة تعيين ملاك البيانات (DC-KPI-02)", 14, True, NAVY, height=24)
r = merged_text(r, "دليل داعم — DC.M.3-E02", 11, True, GOLD, italic=True, height=18)
r = spacer(r, 10)
...
r = section_heading(r, "مجموعات البيانات وملاكها (مصدرها سجل DC.M.5)")
r = spacer(r, 4)
...
headers = [
    ...
    "مالك البيانات",
    "إجمالي المجموعات",
    "عدد المجموعات ذات المالك المحدد",
    "معادلة الحساب",
    "النتيجة النهائية",
]
header_row = r
...
for i, (n, name, owner) in enumerate(DATASETS_FROM_DC_M5):
    row_fill = LIGHT_GREY if i % 2 == 0 else WHITE
    row_vals = [
        f"{n:02d}", name, owner,
        TOTAL_DATASETS, OWNED_DATASETS, FORMULA_TEXT, FINAL_RESULT,
    ]
...
note1 = ws.cell(
    row=r, column=1,
    value=(
        "ملاحظة: أعمدة «إجمالي المجموعات»، «عدد المجموعات ذات المالك المحدد»، «معادلة الحساب»، و«النتيجة "
        "النهائية» تمثل نتيجة الاحتساب الإجمالي الواحد (20 ÷ 20 × 100 = 100%)، وهي مكرَّرة عبر كل صف لتسهيل "
...
review_cell = ws.cell(
    row=r, column=1,
    value="تمت مراجعة هذا السجل من مكتب إدارة البيانات، وتم التحقق من مطابقة أسماء مجموعات البيانات وملاكها الواردة فيه لمحتوى سجل DC.M.5 دون أي إضافة أو حذف أو تعديل.",
)
...
note2 = ws.cell(
    row=r, column=1,
    value="ملاحظة: هذا الملف دليل داعم مستقل ضمن حزمة أدلة تصنيف البيانات الخاصة بهيئة الخدمات الحكومية الذكية (SGSA)، أُعدّ لتوثيق بيانات احتساب مؤشر DC-KPI-02 دون تعديل أي محتوى في تقرير DC.M.3 أو سجل DC.M.5.",
)
...
# ============ Column widths ============
widths = [12, 26, 20, 12, 16, 40, 12]
for i, w in enumerate(widths, start=1):
    ws.column_dimensions[get_column_letter(i)].width = w

wb.save(OUT_PATH)
print("Saved:", OUT_PATH)
print("Rows:", len(DATASETS_FROM_DC_M5), "Total:", TOTAL_DATASETS, "Owned:", OWNED_DATASETS, "Result:", FINAL_RESULT)
