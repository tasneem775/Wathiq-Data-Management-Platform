# -*- coding: utf-8 -*-
"""PDF twin of DC.M.3-E03_Training_Awareness_KPI_Evidence_Record.docx — same content,
same Wathiq/SGSA identity. Evidence Record (not a Calculation Record) for DC-KPI-03 (DC.M.3),
sourced exclusively from the real DC.M.2.docx status report.

Long Arabic paragraphs are pre-wrapped manually (wrap_ar) before being handed to reportlab,
to avoid the reversed-line-order defect discovered and fixed in DC.M.3-E01/E02.
"""
...
OUT_PATH = "06_DC_M3_KPI_Evidence/DC.M.3-E03_Training_Awareness_KPI_Evidence_Record.pdf"
...
title_tbl = Table(
    [[Paragraph(ar("منصة ذكية لإدارة وقياس الامتثال لمتطلبات المكتب الوطني لإدارة البيانات (NDMO)"), subtitle_bar_style)],
     [Paragraph(ar("سجل إثبات مؤشر إكمال التوعية بتصنيف البيانات (DC-KPI-03)"), title_style)]],
    colWidths=[doc.width],
)
...
story.append(Paragraph(ar("سجل إثبات مؤشر إكمال التوعية بتصنيف البيانات (DC-KPI-03)"), report_title_style))
story.append(Paragraph(ar("Training Awareness KPI Evidence Record"), draft_style))
...
story.append(Spacer(1, 4))
story.append(meta_table_ar([
    ("المصدر", "DC.M.2 — تقرير حالة تنفيذ خطة تصنيف البيانات"),
    ("فترة التغطية", "حتى يونيو 2026"),
    ("القيمة الواردة", "90%"),
]))
story.append(Spacer(1, 8))
story.append(Paragraph(ar("البيانات المتعلقة الواردة في DC.M.2:"), ParagraphStyle("BoldLabel", fontName="Arabic-Bold", fontSize=10, textColor=colors.HexColor("#262626"), alignment=2)))
story.append(Spacer(1, 4))
for b in [
    "نسبة إنجاز برنامج التوعية والتدريب: 90%",
    "عدد المشاركين: 47 موظفاً",
    "تنفيذ 3 من أصل 4 ورش توعية مخططة",
]:
...
story.append(Spacer(1, 4))
story.append(ar_p("يعرض الجدول التالي البيانات كما وردت حرفياً في DC.M.2:", body_style, FULL_W))
story.append(Spacer(1, 6))
...
# ---- 5. التسلسل الزمني ----
story.append(Paragraph(ar("5.  التسلسل الزمني"), section_style))
story.append(Spacer(1, 4))
for b in [
    "DC.M.2 وDC.M.3 يعكسان دورة القياس حتى يونيو 2026، وكانت القيمة 90%.",
    "DC.M.4 يمثل دورة قياس لاحقة بعد إجراءات تحسين، وذكر ارتفاع القيمة إلى 95%.",
    "لا يوجد تعارض بين القيمتين لاختلاف فترة القياس.",
]:
    story.append(ar_p("•  " + b, bullet_style, FULL_W - 10))
story.append(Spacer(1, 16))
