# -*- coding: utf-8 -*-
"""Builds a NEW, standalone supporting EVIDENCE record for DC.M.3 (DC-KPI-04) within the
Wathiq/SGSA portfolio context — independent from any existing evidence_repository file.

IMPORTANT — this is an Evidence Record, NOT a Calculation Record: DC.M.4 (the sole approved
source used here) states the KPI's baseline (65%) and current value (88%) verbatim, and
explicitly cites DC.M.3 as "مرجع مؤشرات الأداء" in its own header, but never states the
raw case counts (numerator/denominator) needed to reapply the KPI's formula. This document
documents that 88%/65% are stated in DC.M.4, and does not claim to recompute them.
DC.M.12 was reviewed and explicitly excluded (different KPIs, different maturity question
DC.MQ.3). No Wathiq-package illustrative file is used. Neither DC.M.3 nor DC.M.4 is modified.
"""
import docx
...
OUT_PATH = "06_DC_M3_KPI_Evidence/DC.M.3-E04_Compliance_Procedures_KPI_Evidence_Record.docx"
...
rtl(p)
p.alignment = WD_ALIGN_PARAGRAPH.RIGHT
run = p.add_run("سجل إثبات مؤشر الالتزام بإجراءات تصنيف البيانات (DC-KPI-04)")
run.font.size = Pt(18)
run.font.bold = True
...
add_heading_ar(doc, "سجل إثبات مؤشر الالتزام بإجراءات تصنيف البيانات (DC-KPI-04)", size=14.5)
add_paragraph_ar(doc, "Compliance Procedures KPI Evidence Record", size=11, italic=True, color=GOLD)
...
add_meta_row(meta_table, "رمز الدليل", "DC.M.3-E04")
add_meta_row(meta_table, "المتطلب المرتبط", "DC.M.3 — تقرير مراقبة تنفيذ خطة تصنيف البيانات عبر مؤشرات الأداء الرئيسية")
add_meta_row(meta_table, "المؤشر المرتبط", "DC-KPI-04 — نسبة الالتزام بإجراءات تصنيف البيانات")
...
add_meta_row(meta2, "المصدر", "DC.M.4 — تقرير متابعة تنفيذ خطة تصنيف البيانات")
add_meta_row(meta2, "مرجع المؤشرات", "DC.M.3")
add_meta_row(meta2, "فترة القياس", "حسب دورة التقرير")
add_meta_row(meta2, "خط الأساس", "65%")
add_meta_row(meta2, "القيمة الحالية", "88%")

doc.add_paragraph()

# ============ 4) إثبات القيمة ============
add_heading_ar(doc, "3.  إثبات القيمة")
add_paragraph_ar(doc, "يعرض الجدول التالي البيانات كما وردت حرفياً في DC.M.4:")
make_table(
    doc,
    ["البند", "القيمة", "المصدر"],
    [
...
# ============ 6) التحقق من المصادر ============
add_heading_ar(doc, "5.  التحقق من المصادر")
add_paragraph_ar(
    doc,
